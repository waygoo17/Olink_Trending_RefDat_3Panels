...
- any extra columns are preserved in the output Sample_List sheet

Output Excel workbook contains:
- Sample_List: randomized samples with assigned wells
- Plate_Map: 8x5 plate layout with Sample_ID / Group in each well
- Legend: group-to-color mapping

This version:
- Works on Windows in VS Code
- Uses paths relative to this script
...
# User settings
# -----------------------------
INPUT_CSV = Path(r"C:\Users\wei.guo2\Python\datasheet\Well_template.csv")
OUTPUT_XLSX = Path(r"C:\Users\wei.guo2\Python\datasheet\randomized_plate_map_GroupInfo.xlsx")
SEED = 42  # Change this for a different randomization
...
def group_color_map(groups, seed=42):
    ...
    for grp, h in zip(unique_groups, hues):
        # Pastel-ish colors: high lightness, moderate saturation
...
def write_excel(df, output_xlsx, seed, rows, cols):
    wb = Workbook()
    wb.remove(wb.active)

    group_map = group_color_map(df.loc[df["Group"].ne(""), "Group"].tolist(), seed=seed)

    # -----------------
    # Sample_List sheet
    # -----------------
    ws1 = wb.create_sheet("Sample_List")
    extra_cols = [c for c in df.columns if c not in {"Well", "Sample_ID", "Group"}]
    ...
    matrix = make_plate_matrix(df, rows, cols)
    ...
    for row_idx, r in enumerate(rows, start=3):
        ...
        for col_idx, c in enumerate(["{:02d}".format(x) for x in cols], start=2):
            ...
            fill_color = None
            if value and "(Group " in value:
                grp = value.split("(Group ", 1)[1].split(")", 1)[0].strip()
    ...
    print("Sheets created: Sample_List, Plate_Map, Legend")
...
def main():
    ...
    df = load_samples(INPUT_CSV)
    randomized = assign_randomized_wells(df, SEED, ROWS, COLS)
    write_excel(randomized, OUTPUT_XLSX, SEED, ROWS, COLS)
