...
OUTPUT_DIR = Path(r"C:\Users\wei.guo2\Python\datasheet\olink_t48_results_comprehensive")
...
def plot_panel_trend_summary(log2_mat, pct_mat, panel_name, panel_dir, slug):
    ...
    plt.legend()
    savefig(panel_dir / f"{slug}_panel_level_log2_summary.png")
    ...
    plt.legend()
    savefig(panel_dir / f"{slug}_panel_level_percent_summary.png")
...
def plot_adjacent_change_heatmap(adj_mat, title, outpath, label, figsize=(12, 10)):
...
def analyze_panel(df_panel, panel_name, outdir):
    ...
    lloq_adj_log2 = adjacent_log2_change_matrix(lloq_mat)
    uloq_adj_log2 = adjacent_log2_change_matrix(uloq_mat)
    lloq_adj_pct = adjacent_percent_change_matrix(lloq_mat)
    uloq_adj_pct = adjacent_percent_change_matrix(uloq_mat)

    # Existing raw all-target heatmaps (kept)
    plot_heatmap(
        lloq_mat,
    ...
    # NEW: clustered raw all-target heatmaps under each panel
    ...
    plot_panel_trend_summary(lloq_log2, lloq_pct, panel_name, panel_dir, slug)
    plot_panel_trend_summary(uloq_log2, uloq_pct, panel_name, panel_dir, slug)
    ...
    plot_adjacent_change_heatmap(
        ...
        figsize=(12, max(8, 0.22 * len(lloq_adj_log2.index))),
    )
    plot_adjacent_change_heatmap(
        uloq_adj_log2,
        ...
        label="log2 change vs previous version",
        figsize=(12, max(8, 0.22 * len(uloq_adj_log2.index))),
    )
    plot_adjacent_change_heatmap(
        lloq_adj_pct,
        ...
        figsize=(12, max(8, 0.22 * len(lloq_adj_pct.index))),
    )
    plot_adjacent_change_heatmap(
        uloq_adj_pct,
        ...
        label="% change vs previous version",
        figsize=(12, max(8, 0.22 * len(uloq_adj_pct.index))),
    )
