#!/usr/bin/env python3
"""
Olink T48 reference range analysis for Windows / VS Code

Expected CSV columns:
Panel, PanelVersion, Assay, LLOQ, ULOQ

Outputs:
- version summary tables
- heatmaps for LLOQ and ULOQ
- fold-change heatmaps vs first version
- boxplots by version
- assay-level line plots
- dynamic range plots
- top-changes tables
- first-vs-last volcano-style plot

Run in VS Code terminal:
python 3panelsTrending.py
"""
...
OUTPUT_DIR = r"C:\Users\wei.guo2\Python\datasheet\olink_t48_results2"
...
def normalize_panel_version(x: Any) -> Any:
    """
    Normalize panel version values so sorting works reliably.
...
def version_sort_key(v: Any):
    """
    Stable sorting key for mixed version types.
    Numeric versions sort before text versions.
    """
    if pd.isna(v):
        return (2, "")
...
def panel_slug(panel_name: str) -> str:
    return (
        str(panel_name)
        .lower()
...
def parse_panel_versions(series: pd.Series) -> List[Any]:
...
def make_matrix(df: pd.DataFrame, value_col: str, version_order: List[Any]) -> pd.DataFrame:
...
def summarize_version_stats(df_panel: pd.DataFrame, value_col: str) -> pd.DataFrame:
    return (
...
def plot_boxplot(df_panel: pd.DataFrame, value_col: str, title: str, outpath: Path, show_points: bool = False) -> None:
    ...
    plt.boxplot(data, labels=[version_str(v) for v in versions], showfliers=False)
    ...
    if show_points:
        ...
        for i, vals in enumerate(data):
            if len(vals) == 0:
                continue
            x = rng.uniform(i + 0.75, i + 1.25, size=len(vals))
            plt.scatter(x, vals, alpha=0.5, s=15, color="steelblue", zorder=3)
...
def plot_dynamic_range(df_panel: pd.DataFrame, outpath: Path) -> None:
    versions = parse_panel_versions(df_panel["PanelVersion"])
    rows = []
...
def top_change_table(mat: pd.DataFrame, metric_name: str, n: int = 15) -> pd.DataFrame:
    first_col = mat.columns[0]
...
def plot_volcano(df_panel: pd.DataFrame, outpath: Path) -> pd.DataFrame:
    versions = parse_panel_versions(df_panel["PanelVersion"])
    first_v = versions[0]
    last_v = versions[-1]
...
def analyze_panel(df_panel: pd.DataFrame, panel_name: str, outdir: Path) -> None:
...
def main() -> None:
    ...
    overall_summary = (
        ...
        .reset_index()
        .sort_values(["Panel", "PanelVersion"], key=lambda col: col.map(version_sort_key) if col.name == "PanelVersion" else col)
    )
    overall_summary.to_csv(outdir / "overall_version_summary.csv", index=False)

    for panel_name, df_panel in df.groupby("Panel", sort=False):
        analyze_panel(df_panel, panel_name, outdir)

    files = []
    for root, _, filenames in os.walk(outdir):
        for fn in filenames:
            files.append(os.path.relpath(os.path.join(root, fn), outdir))
    pd.DataFrame({"output_files": sorted(files)}).to_csv(outdir / "output_manifest.csv", index=False)
